...
export class formParameters{
  constructor(Nombre, DNI, Provincia, Municipio, Direccion, Telefono, Correo, Acepta, Firma){
    this.Nombre = Nombre;
    this.DNI = DNI;
...
export async function modifyPdf(canvasScreenshotURL, formParameters) {
    ...
    //const helveticaFont = await pdfDoc.embedFont(StandardFonts.Tahoma)
        ...
        const fontBytes = await fetch("TAHOMA_0.TTF").then(res => res.arrayBuffer());
    ...
    const formDataText = formParameters.Nombre + " , " + formParameters.DNI + " , " + formParameters.Direccion + ", " + formParameters.Municipio + ", " + formParameters.Provincia + " , " + formParameters.Telefono + " , " + formParameters.Correo;
...
*/
    ...
    //TEXTO/SCREENSHOT
    const screenshotPNGBytes = await fetch(canvasScreenshotURL).then((res) => res.arrayBuffer())
    ...
    //FIRMA
    const firmaPNGBytes = await fetch(formParameters.Firma).then((res) => res.arrayBuffer())
    ...
    var file = new File([pdfBytes], "CA ATLAS - SOLICITUD SOCIO " + formParameters.Nombre + ".pdf", {type: "application/pdf"});
